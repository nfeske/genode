                     ...
                     uint64_t u64Arg, PSUPVMMR0REQHDR pReqHdr)
{
	...
	case VMMR0_DO_GMM_ALLOCATE_PAGES:
	{
		...
		//
		// XXX Find out what's behind this assertion. It triggers because
		// XXX PGM_HANDY_PAGES = 128, ONE_PAGE_SIZE = 4096, GMM_CHUNK_SIZE = 1<<21
		//
                             ...
                             RTGCPHYS const GCPhys, RTGCUINT vbox_fault_reason)
{
	...
	if (PGM_PAGE_GET_TYPE(pPage) == PGMPAGETYPE_ROM &&
	    vbox_fault_reason & VMX_EXIT_QUAL_EPT_DATA_WRITE) {
	...
	Assert(!(vbox_fault_reason & VMX_EXIT_QUAL_EPT_ENTRY_PRESENT));
	...
	if (PGM_PAGE_GET_TYPE(pPage) == PGMPAGETYPE_MMIO2 && handler) {
		...
		PGMACCESSTYPE access_type = (vbox_fault_reason & VMX_EXIT_QUAL_EPT_DATA_WRITE) ? PGMACCESSTYPE_WRITE : PGMACCESSTYPE_READ;
