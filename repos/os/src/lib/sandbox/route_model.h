...
class Sandbox::Route_model : Noncopyable
{
	public:
		...
		class Rule : Noncopyable, List<Rule>::Element
		{
			private:
				...
				struct Selector
				{
					...
					void print(Output &out) const
					{
						...
						print(out, "type=", (int)type, " label_checksum=", label_checksum.value);
					}
				};
				...
				bool const _specific_service; /* not <any-service> */
				...
				:
					_alloc(alloc),
					_node(node), _selector(node),
					_service_checksum(node.attribute_value("name", Service::Name())),
					_specific_service(node.has_type("service"))
				{
			...
			public:
				...
				Child_policy::Route with_targets(FN const &fn) const
				{
	...
	public:
		...
		template <typename FN>
		Child_policy::Route with_targets(Query const &query, FN const &fn) const
		{
			for (Rule const *r = _rules.first(); r; r = r->next())
				if (r->matches(query)) {
					try {
						return r->with_targets(fn);
					}
