...
struct Test::Main
{
	...
	struct Colors
	{
		...
		Pixel_rgb888 values[N] { };

		struct Start { unsigned hue, brightness; };
		Colors(Start start)
		{
			unsigned h = start.hue>>8, b = start.brightness>>8;
			for (unsigned i = 0; i < N; i++) {

				h = (h + 1) & 0xff;
				b = (b + 2) & 0x1ff;

				unsigned const v = (b >= 0x100) ? b - 0x100 : 0xff - b;

				Color const c = color_from_hsv(h, 255, v);

				values[i] = Pixel_rgb888(c.r, c.g, c.b);
			}
		}
	};

	Colors::Start _color_anim { };
	...
	void _handle_timer()
	{
		...
		Colors const colors { _color_anim };
		_color_anim.hue += 256;
		_color_anim.brightness += 44;
		...
		_capture_input->with_texture([&] (Texture<Pixel> const &texture) {
			...
			_output->with_surface([&] (Surface<Pixel> &surface) {
				...
				(void)affected;
			 	...
			 	unsigned const w = 1080/2;
			 	unsigned const h = 480;
			 	int      const x_off = (640 - int(w))/2;

				surface.clip({ { 0, 0 }, { w, h } });

				Blit_painter::paint(surface, texture, Gui::Point(-x_off, 0));

				Pixel_rgb888 const *src = texture.pixel() + x_off;
				Pixel_rgb888       *dst = surface.addr()  + w;
				unsigned const src_w = texture.size().w;
				unsigned const dst_w = surface.size().w;

				for (unsigned y = 0; y < h; y++) {

					Pixel_rgb888 const *s = src;
					Pixel_rgb888       *d = dst;

					for (unsigned x = 0; x < w; x++, s++, d++) {

						uint8_t green = (s->pixel >> 8) & 0xff;
						*d = colors.values[green];
					}
					src += src_w;
					dst += dst_w;
				}
				_output->_gui.framebuffer.refresh({ { }, { 1080, h } });
			});
		});
