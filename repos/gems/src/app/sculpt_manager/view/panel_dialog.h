...
struct Sculpt::Panel_dialog
{
	...
	Expanding_reporter _dialog_reporter { _env, "dialog", "panel_dialog" };

	Attached_rom_dataspace _hover_rom { _env, "panel_view_hover" };

	Signal_handler<Panel_dialog> _hover_handler {
		_env.ep(), *this, &Panel_dialog::_handle_hover };
	...
	void _handle_hover()
	{
		...
		bool const changed =
			_item.match(hover, "dialog", "frame", "float", "hbox", "button", "name");
	...
	Panel_dialog(Env &env, Allocator &alloc, State const &state)
	:
		_env(env), _alloc(alloc), _state(state)
	{
