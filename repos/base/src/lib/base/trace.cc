	...
	if (enabled && (new_policy || policy_module == 0)) {
		...
		_env().rm().attach(policy_ds, Region_map::Attr {
			...
			[&] (Region_map::Range range)  {
				policy_module = reinterpret_cast<Policy_module *>(range.at); },
		...
		_env().rm().attach(buffer_ds, attr).with_result(
			[&] (Region_map::Range range) {
				buffer = reinterpret_cast<Buffer *>(range.at); },
		...
		if (!myself && !main_trace_control) {
			...
			if (ds.valid()) {
				...
				_env().rm().attach(ds, attr).with_result(
					[&] (Region_map::Range range) {
						main_trace_control = reinterpret_cast<Trace::Control *>(range.at); },
