...
struct Linker::Elf_file : File
{
	...
	void _allocate_region_within_linker_area(Name const &name)
	{
		...
		reloc_base = 0;
		allocated_region.with_result(
			[&] (addr_t base) { reloc_base = base; },
			[&] (Region_map::Alloc_region_error) {
	...
	{
		Region_map::r()->attach(rom_cap, Region_map::Attr {
			.size       = round_page(p.p_memsz),
			.offset     = off_t(trunc_page(p.p_offset)),
			.use_at     = true,
	...
	void load_segment_rw(Elf::Phdr const &p, int nr)
	{
		void * const src = env.rm().attach(rom_cap, Region_map::Attr {
			.size       = { },
			.offset     = off_t(p.p_offset),
			.use_at     = { },
			.at         = { },
			.executable = { },
			.writeable  = true
		}).convert<void *>(
			[&] (Genode::Region_map::Range range)  { return (void *)range.at; },
